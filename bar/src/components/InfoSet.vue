  ...
  <div class="InfoSet">
    <div class="Info">
      ...
      <div class="user-data">
        ...
        <div>
          昵&nbsp;&nbsp;&nbsp;&nbsp;称:
          <span v-show="is_active">{{ showData(nick_name) }}</span>
          <el-input
            v-show="!is_active"
            class="info_input"
            placeholder="请输入昵称"
        ...
        <!-- 身份证 -->
        <div>
          身份证:<span v-show="!is_id_card">{{ showData(id_card) }}</span>
          <el-input
            v-show="is_id_card"
            class="info_input"
            placeholder="身份证只能设置一次，请谨慎填写"
            v-model="id_card"
            clearable
          ></el-input>
        </div>
  ...
  methods: {
    ...
    preInfo() {
      if (confirm("请确认保存资料吗？")) {
        //    通过跨域来提交数据
        axios
          .get("http://localhost:8080/django/user/info", {
            params: {
