<template>
    <div class="InfoSet Theme">
        <div class="Info"><p class="title">设置主题详细资料</p>
            ...
            <p>主题名 <span v-show="is_active">:{{barData.bar_name}}</span>
                <el-input v-show="!is_active" class='info_input' placeholder="请输入昵称" v-model="barData.bar_name"
                          clearable disabled></el-input>
            </p>
            <p>主题主<span v-show="is_active">:{{barData.user}}</span>
                <el-input v-show="!is_active" class='info_input' placeholder="请输入性别" v-model="barData.user"
                          clearable disabled></el-input>
            </p>
            <p>身份证<span v-show="is_active">:{{barData.id_card}}</span>
                <el-input v-show="!is_active" class='info_input' placeholder="身份证只能设置一次，请谨慎填写" v-model="barData.id_card"
                          clearable disabled></el-input>
            </p>
            <p>简介<span v-show="is_active">:{{barData.int}}</span>
                <el-input v-show="!is_active"
                          class='bar_input'
                          type="textarea"
                          :rows="2"
                          placeholder="请输入内容"
    ...
    export default {
        ...
        methods: {
            ...
            getBarData() {
                ...
                // 跨域请求资料
                axios.get('/django/article/getBarData', {
                    params: {
                        bar_id: this.bar_id
                    }
                }).then(res => {
            ...
            lookPortrait() {
                ...
                if (!this.uploadImg) {
                    return '/django/media/' + this.barData.portrait
                }
            ...
            handleFileChange() {
                ...
                formData.append('bar_id', this.bar_id)
                ...
                axios({
                    method: 'post',
                    url: '/django/article/postBarPortrait',
                    // params: {
                    //     bar_id: this.bar_id
                    // },
                    data: formData,
            ...
            //保存主题简介
            preInfo() {
                if (confirm('请确认保存主题简介吗？')) {
                    //    通过跨域来提交数据
                    axios.get('/django/article/setBar', {
                        params: {
                            bar_id: this.bar_id,
                            int: this.barData.int
                        }
                    }).then(res => {
                        if (res.data['code'] === 200) {
                            alert(res.data['message'])
